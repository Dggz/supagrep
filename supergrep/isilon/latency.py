...
def process(workbook: Any, content: list) -> list:
    """Process Latency worksheet

    :param workbook:
    :param content:
    :return:
    """
    worksheet = workbook.get_sheet_by_name('Latency')

    rows, header = [], []  # type: list, list
    for csv_file in content:
        cluster = perf_cluster_name(csv_file)
        latency_csv = csv.reader(csv_file.split('\n')[1:])
        header = ['Cluster'] + next(latency_csv) if header == [] else header
        rows += [[cluster] + csv_row
                 for csv_row in latency_csv if csv_row not in ([], header[1:])]
