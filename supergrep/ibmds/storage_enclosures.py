...
def process(workbook: Any, content: list) -> None:
    """Process Storage Enclosures worksheet

    :param workbook:
    :param content:
    """
    worksheet = workbook.get_sheet_by_name('Storage Enclosures')

    header = [
        'SystemName', 'ID', 'Frame', 'State', 'Number of Drives',
        'Drive Capacity (GB)', 'Drive Class', 'Install Time', 'S/N', 'Location'
    ]
    ...
    for csv_file in content:
        ...
        storage_rows = get_rows(storage_csv, header[1:])
        rows += [system_name[0] + feat_row for feat_row in storage_rows]
